...
export class InteractionManager {
  ...
  addTextAnnotation(event, pageNum) {
    ...
    setTimeout(() => {
      if (textAnnotation.textEl) {
        ...
        range.selectNodeContents(textAnnotation.textEl);
  ...
  handleTextMouseDown(event) {
    ...
    if (isRotateHandle) {
      ...
      if (annotation.textEl.matches(':focus')) {
        return; // Let browser handle text editing
      }
  ...
  startDragging(event, annotation) {
    ...
    // Store offset from annotation's stored position (not transformed rect)
    this.dragStartPos = {
